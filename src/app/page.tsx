...
export default function Home() {
  ...
  async function processDecision() {
    ...
    try {
      ...
      // IMPORTANT:
      // Keep this receipt permanently associated with this case
  ...
  async function verify(target: Evidence) {
    ...
    try {
      ...
      // Update only the currently displayed verification.
      //
      // We intentionally do NOT overwrite the original stored
      // verification here when running a tamper test.
  ...
  async function simulateAlteration() {
    ...
    // The tampered receipt becomes the currently displayed
    // diagnostic receipt.
    setEvidence(changed);
    setModified(true);
  ...
  function selectCase(item: CaseItem) {
    ...
    setOriginalEvidence(savedEvidence);
    setVerification(savedVerification);

    // Every time we switch cases, start a fresh diagnostic state.
    // This means a tamper test on one record does not affect
    // another record.
    setModified(false);
