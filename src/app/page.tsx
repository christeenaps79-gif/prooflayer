...
export default function Home() {
  ...
  async function processDecision() {
    ...
    try {
      ...
      const auditResponse = await fetch(`/api/evidence?recordId=${encodeURIComponent(data.recordId)}`);
      const auditData = await auditResponse.json();
      if (auditResponse.ok && auditData.success) {
        setAuditConfirmed(true);
        setAuditRecord(auditData.record);
        addTimeline(`Receipt persisted and retrieved from server store`);
      }
    } catch (err) {
